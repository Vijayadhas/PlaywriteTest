...
const PROCESSOR_HEATSINK_CHOICE = {
  sectionName: 'Processor',
  sectionSelector: '#section_header_processor, #section_header_processorSection, [id*="section_header" i][id*="processor" i]',
  choiceName: 'Choice Heatsink Kit',
...
export const dl360Gen12: ModelSpec = {
  ...
  async run(oca: OcaConfigPage, _result: EndBomResult): Promise<void> {
    ...
    await oca.ensureChoiceItemStable(PROCESSOR_HEATSINK_CHOICE);
    ...
    await oca.reapplyChoiceItemAfterRecalculation(PROCESSOR_HEATSINK_CHOICE, 'Memory selection');
    await page.waitForTimeout(3_000);

    await oca.selectComponentSection('Power Supplies', '#section_header_power');
    await oca.setRowQuantity(POWER_ROW, '1');
    await oca.reapplyChoiceItemAfterRecalculation(PROCESSOR_HEATSINK_CHOICE, 'Power Supply selection');
