...
export class OcaConfigPage {
  ...
  async setRowQuantity(rowId: string, quantity: string) {
    ...
    // native select (memory / power path) with selectOption + label fallback
    const dropdown = row.locator('select').first();
    if (await dropdown.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await dropdown.selectOption(quantity).catch(async () => {
        await dropdown.selectOption({ label: quantity });
      });
      await waitForBlockingOverlay(this.page);
      await expect(dropdown).toHaveValue(quantity, { timeout: 10_000 }).catch(() => undefined);
      return;
    }

    // DOM-by-value path (processor's special "set via DOM by value")
    const setViaDom = await row.evaluate((element, quantity) => {
      const select = element.querySelector<HTMLSelectElement>('select');
      if (select) {
        const option = Array.from(select.options).find((item) => item.value.trim() === quantity || item.textContent?.trim() === quantity);
        if (!option) {
          return false;
        }
      ...
      return element.querySelector('.item_qty_div')?.textContent?.trim() === quantity;
    }, quantity);
    if (setViaDom) {
      await waitForBlockingOverlay(this.page);
      return;
    ...
    // .item_qty_div click + [id="<qty>"] option fallback
    const quantityControl = row.locator('.item_qty_div, .item_qty').first();
    await quantityControl.waitFor({ state: 'visible', timeout: 10_000 });
    if ((await quantityControl.textContent())?.replace(/\s+/g, ' ').trim() === quantity) {
      return;
    }
    ...
    const option = this.page.locator(`[id="${quantity}"]`).first();
    if (await option.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await option.click({ force: true });
      await waitForBlockingOverlay(this.page);
      return;
    }
    throw new Error(`Quantity option ${quantity} was not available for ${rowId}`);
  }
  ...
  private async saveOcaConfigDialog() {
    ...
    if (await saveDialog.isVisible({ timeout: 10_000 }).catch(() => false)) {
      const clicStatus = await saveDialog.locator('#clicStatus').textContent().catch(() => '');
      console.log(`[INFO] Save dialog remained open after Save click. CLIC Status: ${(clicStatus ?? '').trim() || 'not available'}`);
      await saveDialog.locator('.ui-dialog-titlebar-close').click({ force: true });
    }
    await saveDialog.waitFor({ state: 'hidden', timeout: 60_000 });
    await waitForBlockingOverlay(this.page);
  }
