...
const SMART_CHASSIS_TEXT =
  'Includes 1x 12EDSFF drive cage, 1x primary x16/x16/x16 riser, ' +
  '1x secondary x16/x16/x16 riser, 2x x8 riser enablement cable kits, ' +
  '1x CPU1-to-Rear OCP Slot A x16 cable, 1x CPU2-to-Rear OCP Slot B x16 cable, ' +
  'and 1x 12EDSFF OCP Direct Attach PCIe FIO bundle per server.';

const DEFAULT_PROCESSOR_RESULT = 'Default / unchanged';
const DEFAULT_MEMORY_RESULT = 'Default / unchanged';
...
test('configure OCA Config 2 third CTO model P73286-B21 automation', async ({ playwright }) => {
  test.setTimeout(20 * 60_000);

  let context: BrowserContext | undefined;

  const result: ThirdModelExcelResult = {
    modelNo: BASE_MODEL_PART,
    processor: DEFAULT_PROCESSOR_RESULT,
    memory: DEFAULT_MEMORY_RESULT,
  ...
  try {
    console.log('[START] Third model automation started');
    ...
    const flow = new ThirdModelFlow(page, context, result);
    ...
    await test.step(
      'Select Smart Chassis 12EDSFF OCP Direct Attach PCIe FIO bundle',
      async () => {
    ...
    console.log('[PASS] Third model automation completed successfully');
  } catch (error) {
    console.error('[FAIL] Third model automation failed:', error);
...
class ThirdModelFlow {
  ...
  private async selectCtoBaseModelFilterIfAvailable(partNumber: string) {
    ...
    const ctoBaseVisible =
      ...
      (currentDiagnostics as any).matchingCards.some((card: any) =>
        /CTO Base Model|["“]?\s*P73286-B21["”]?\s*[—-]\s*CTO\b|CTO Compute/i.test(card.text ?? ''),
      );
  ...
  private async findCustomizeButtonForCard(card: Locator, partNumber: string) {
    ...
    const buttonHandleFound = await this.page.evaluate((partNumber) => {
      ...
      const cards = Array.from(document.querySelectorAll<HTMLElement>('#dqe_cards_container .dqe-card, #dqe_cards_container div'))
        ...
        .filter((element) => /CTO Base Model|CTO Compute|["“]?\s*P73286-B21["”]?\s*[—-]\s*CTO\b/i.test(normalize(element.textContent)));
  ...
  async selectSmartChassis() {
    await this.openSection(
      'Smart Chassis',
      '#section_header_smartChassis, #section_header_smartChassisSection, [id*="section_header" i][id*="smartChassis" i]',
    );

    const exactRecordedRow = this.page
      .getByRole('row', {
        name: new RegExp(
          'Includes 1x 12EDSFF drive cage.*12EDSFF OCP Direct Attach PCIe FIO bundle per server',
          'i',
        ),
      })
      .filter({ visible: true })
    ...
    const fallbackRow = this.page
      ...
      .filter({ hasText: '12EDSFF OCP Direct Attach PCIe FIO bundle per server' })
      .filter({ visible: true })
      .first();

    const row = (await exactRecordedRow.isVisible({ timeout: 8_000 }).catch(() => false))
      ? exactRecordedRow
      : fallbackRow;

    await expect(
      row,
      `Third model Smart Chassis row should be visible: ${SMART_CHASSIS_TEXT}`,
    ).toBeVisible({ timeout: 60_000 });

    console.log(
      '[DEBUG] Third model Smart Chassis row:',
      await this.debugLocatorDetails(row, 'Third model Smart Chassis'),
    );

    await this.selectRowRadio(row, 'Third model Smart Chassis 12EDSFF bundle');
  }
